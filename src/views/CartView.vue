  ...
  <div class="min-h-screen bg-gray-50">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div class="bg-white rounded-2xl shadow-lg overflow-hidden">
            <div v-for="(item, index) in cartStore.items" :key="item.id"
              ...
              <div class="flex-1 ml-4 md:ml-6">
                ...
                <p class="text-indigo-600 font-medium mt-1">{{ formatCurrency(item.price) }}</p>
              </div>
            ...
            <div class="space-y-4">
              ...
              <div class="border-t pt-4">
                <div class="flex justify-between text-lg font-bold text-gray-900">
                  <span>Tổng cộng</span>
                  <span class="text-indigo-600">{{ formatCurrency(cartStore.totalPrice) }}</span>
                </div>
              </div>
            </div>

            <button @click="handleCheckout"
              class="w-full mt-6 py-4 bg-indigo-600 text-white rounded-xl font-bold text-lg hover:bg-indigo-700 transition-colors">
              Đặt hàng
            </button>
      ...
      <div v-else class="text-center py-20">
        ...
        <router-link to="/shop"
          class="inline-flex items-center px-6 py-3 bg-indigo-600 text-white rounded-xl font-medium hover:bg-indigo-700 transition-colors">
          <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
              d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 11-4 0 2 2 0 014 0z" />
          </svg>
    ...
    <Modal :isOpen="isCheckoutModalOpen" @close="closeSuccessModal"
      ...
      <div v-if="isOrderSuccess" class="text-center py-4">
        ...
        <router-link to="/shop" @click="closeSuccessModal"
          class="inline-block px-6 py-3 bg-indigo-600 text-white rounded-xl font-medium hover:bg-indigo-700 transition-colors">
          Tiếp tục mua sắm
        </router-link>
      </div>
